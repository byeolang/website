...
이 문서는 2026-04-03 기준 최신 상태를 반영한 Scene2 인계 문서다.
이제 목적은 "자산 생성 준비"가 아니라, 실제 홈페이지 기준으로 Scene2를 완성하는 데 필요한 현재 상태와 다음 작업을 정리하는 것이다.

중요:

- Scene2는 더 이상 "미구현 예약 구간"이 아니다.
- 1차 정적 레이아웃은 실제 코드베이스에 이미 붙어 있다.
- 현재 남은 핵심은 애니메이션 구현과 최종 반응형 정리다.

## 현재 결론 요약

- Scene1(`TakeOff`)은 실제 홈페이지에 구현되어 있다.
- Scene2(`UprisingRocket`)는 실제 홈페이지에 정적 레이아웃까지 반영되어 있다.
- Scene2용 PNG 자산은 `resource-generation/`뿐 아니라 `assets/images/scenes/`에도 이미 동기화되어 있다.
- Scene2 텍스트는 `data-lang` 기반 키로 분리되어 있다.
- Scene2의 제목 / 설명 / 좌측 그라데이션은 Scene1 표현 방식에 맞춰 정렬 중이며, 현재 정적 기준 레이아웃 검수 단계는 사실상 끝났다.
- 아직 `assets/js/scenes.mjs`의 `UprisingRocket` 애니메이션 구현은 비어 있다.
- 최종 responsive 튜닝은 아직 끝나지 않았다.
...
- 임시 responsive breakpoint 규칙

### JS

- `assets/js/scenes.mjs`에 `class UprisingRocket extends Scene`는 존재한다.
- 하지만 `_onAnimate(tl)`는 아직 비어 있다.

즉 현재 상태는:

- 정적 레이아웃: 구현됨
- 애니메이션: 미구현
- 최종 responsive polish: 미완료
...
가장 중요한 다음 작업이다.
현재 정적 레이아웃을 기준 pose로 삼고, `assets/js/scenes.mjs`의 `UprisingRocket`에 실제 motion을 넣어야 한다.

핵심 작업:

- `UprisingRocket._onAnimate(tl)` 구현
- 로켓의 시작 / 중간 / 종료 pose 정의
- 구름 레이어별 상대 이동량 정의
- copy 등장 / 유지 / 퇴장 시점 정의
- 현재 Scene 엔진의 반전 progress 규칙에 맞춰 수식 재해석

주의:

- 프로토타입의 공식을 그대로 복붙하지 않는다
- 현재 확정된 레이어 의미(`far / deck / foreground / frame / mist`)를 기준으로 motion을 다시 설계한다
- 지금 사용자와 검수한 정적 구도를 중간 기준점으로 삼는 편이 안전하다

### 3단계. 최종 responsive 정리

임시 media query는 이미 들어가 있지만, 그건 baseline 수준이다.
애니메이션이 들어간 뒤에는 다시 조정해야 한다.

최종 responsive 단계에서 확인할 것:

- 모바일에서 로켓과 copy가 겹치지 않는가
- 제목과 description이 너무 좁아지거나 줄바꿈이 깨지지 않는가
- 하단 구름층이 작은 화면에서도 의도대로 보이는가
- 우상단의 열린 공간이 유지되는가
- Scene1 / Scene2 전환 구간에서 레이어 충돌이 없는가
...
4. 하지만 `UprisingRocket` 애니메이션은 아직 비어 있다.
5. 현재 확정된 레이어 의미는 `far / deck / foreground / frame / mist`다.
6. 다음 핵심은 애니메이션 구현, 그 다음이 최종 responsive polish다.

## 마지막 메모

2026-04-03 기준 현재 상태는 "구조 파악" 단계를 이미 지나갔다.
지금부터는 실제 완성 단계다.

즉 다음 작업 우선순위는 아래 순서가 맞다.

1. `UprisingRocket` 애니메이션 구현
2. 실제 스크롤 체감과 전환 검수
3. 반응형 최종 조정
