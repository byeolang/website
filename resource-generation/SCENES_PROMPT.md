...
## 24. 2026-04-18 작업 기록 / 다음 AI 인수인계 메모

이 섹션이 현재 Scene3의 최신 source of truth다. 특히 4/17 메모 이후 실제 구현/피드백이 많이 진행되었으므로, 아래 내용을 우선 기준으로 본다.

### 오늘 실제로 정리된 것
- `assets/js/scene3-space-field.mjs`
  - `large` asteroid는 이제 랜덤 기반이 아니라 **고정 프리셋 상수**로 다룬다.
  - large는 현재 3개만 사용한다.
  - large는 **좌하 / 좌상 / 우측** 시작 분포를 의도적으로 고정해두는 방향이 맞다.
  - large의 시작점 차이는 타이밍 꼼수가 아니라, **각 소행성의 route 직선 위에서 뒤로 얼마나 물러났는지**로 만든다.
  - 즉 `source -> target` 직선을 만든 뒤, 그 직선의 반대방향으로 시작점을 더 물린 좌표를 실제 시작점으로 사용해야 한다.
  - large는 opacity fade-in 없이 시작부터 보여야 한다.
  - large는 현재 사용자 기준으로 대체로 만족할 수준까지 왔고, 이후에는 큰 문법을 흔들지 않는 편이 좋다.
- `assets/js/scene3-space-field.mjs`
  - `small` asteroid는 현재 사용자 기준으로 **아주 좋은 상태**다.
  - small은 **절대 건드리지 않는 것**이 현재 규칙이다.
  - small은 이후 최종적으로도 랜덤이 아니라 고정 상수 프리셋 방식으로 가는 철학이 맞지만, 지금 세션 기준으로는 이미 좋은 상태이므로 수정 대상이 아니다.
- `assets/js/scene3-space-field.mjs`
  - `medium` asteroid는 새로 추가되었다.
  - medium의 정답 문법은 large의 단순 축소판이 아니다.
  - medium은 **small처럼 처음부터 전방 공간 전체에 퍼져 있는 환경 오브젝트**처럼 시작해야 하고,
    스크롤이 진행되면 **large처럼 각자 자기 벡터 방향으로 translate**해야 한다.
  - 즉 medium은
    - 초기 배치: distributed start
    - 이동 문법: directed route translate
    를 동시에 만족해야 한다.
  - 현재 medium도 랜덤이 아니라 **고정 프리셋 상수** 기반으로 다루는 방향이 맞다.
- `assets/js/scenes.mjs`
  - Scene3의 실제 field progress는 timeline progress를 그대로 쓰지 않고,
    `fieldProgress = sceneProgress * 0.78` 형태로 재매핑하고 있다.
  - 이건 스크롤 길이를 줄이기 위한 것이 아니라,
    **같은 스크롤 길이 안에서 Scene3 비행 애니메이션이 조금 더 일찍 끝나게 하기 위한 매핑**이다.

### large 관련 현재 확정 규칙
- large는 3개만 사용한다.
- large는 scale 애니메이션으로 커지면 안 된다.
- large는 opacity fade-in/fade-out에 의존하면 안 된다.
- large는 애니메이션 시작 후에는 **translate only**여야 한다.
- large의 시작 위치 차이는 `progressOffset` 같은 타이밍 속임수로 만들지 말고,
  **route 위 시작점의 물림 거리 차이**로 만들어야 한다.
- large의 시작 위치/방향은 랜덤으로 두지 말고,
  **좌하 / 좌상 / 우측** 같은 읽기 좋은 상수 프리셋으로 직접 설계하는 편이 맞다.
- large가 너무 빨리 사라지면, visible 판정은 반드시
  **실제 startWorld -> targetWorld 전체 route 길이** 기준이어야 한다.
  `routeBackOffset`를 반영하지 않은 base route 길이만 쓰면 중간 vanish가 생긴다.

### medium 관련 현재 확정 규칙
- medium은 지금부터의 핵심 작업 대상이다.
- medium은 small처럼 이미 환경에 퍼져 있어야 한다.
  - 시작 프레임부터 전방 공간에 여러 개가 분포해 있어야 한다.
  - 타겟 행성 주변에서 한 점 출발처럼 보이면 실패다.
- medium은 large처럼 자기 route를 따라 translate 해야 한다.
- medium 역시 랜덤이 아니라 **고정 프리셋 상수**로 배치한다.
- medium은 깊이 차이가 충분히 커야 한다.
  - 사용자가 직접 large에서 `routeBackOffset` 65 같은 큰 값을 사용했듯,
    medium도 생각보다 값 변동성이 크지 않으면 평평하게 읽힌다.
- 하지만 depth variation을 과하게 주면 medium이 화면 가장자리/구석에만 생기고 바로 날아가므로,
  **depth variation과 screen occupancy 사이의 균형**이 중요하다.
- 현재 medium에서 실제로 조정해야 하는 핵심 값은 대체로 다음이다.
  - `sourceDepth`
  - `sourceNormX`, `sourceNormY`
  - `targetDepth`
  - `targetNormX`, `targetNormY`
  - `initialRouteOffset`
  - `worldScale`
- medium은 현재 large보다 작고 느리지만, small보다는 명확히 읽혀야 한다.

### Scene3 종료 타이밍 관련 현재 확정 규칙
- 사용자가 원하는 것은 `행성을 뚫기 전에 정지하는 우주선`이 아니다.
- 사용자가 원하는 것은
  **우주선이 꾸준한 속도로 계속 비행하고 있는데, 행성을 뚫기 직전에 Scene3가 딱 끝나는 것**이다.
- 따라서 해결 방향은
  - 행성 접근 자체를 멈추는 것보다는
  - **같은 스크롤 길이 안에서 field progress를 더 일찍 소모하도록 매핑**하는 쪽이 맞다.
- 현재 `fieldProgress = sceneProgress * 0.78`은 그 방향의 임시/중간 해법이다.
- 이 값은 추후 미세조정 가능하지만,
  **정지 구간을 만드는 clamp 방식은 피해야 한다.**

### 현재 남은 핵심 TODO
1. **Scene3 반응형**
   - 현재 사용자 기준으로 남은 가장 큰 작업은 반응형이다.
   - 우선 검토 결과, 주요 수정 대상은 다음 3축이다.
     - `assets/css/scenes.css`의 `#scene3-copy` / 카드 / 모바일 배치
     - `assets/css/scenes.css`의 rocket / planet fallback DOM 위치
     - `assets/js/scene3-space-field.mjs`의 medium / large mobile-safe 분포 보정
2. medium의 screen occupancy / depth variation 균형 다듬기
   - 전부 border에 붙어 있거나 너무 빨리 날아가면 안 된다.
   - 그렇다고 너무 평평한 한 층처럼 보여도 안 된다.
3. Scene3 종료 타이밍 미세조정
   - 현재는 대체로 맞아졌지만, `0.78` 같은 progress 배율은 화면/기기별 검증 후 추가 미세조정 가능성이 있다.

### 반응형 검토 체크리스트 요약
- `#scene3-copy`
  - 모바일/태블릿에서 title / description / cards의 수직 밀도 재조정
  - 필요 시 `640px` 전후 breakpoint 추가
- rocket / planet fallback
  - 작은 화면에서 copy와 겹치지 않는지 확인
  - `top / right / size / bottom` 값 재조정 가능성 높음
- medium / large
  - 좁은 화면에서 다시 border spawn처럼 보이지 않는지 확인
  - mobile에서는 `sourceNorm / targetNorm / worldScale`를 약간 중앙 쪽 / 보수적으로 압축할 필요가 있을 수 있음
- small
  - 현재 상태 유지. 건드리지 말 것.

### 다음 AI가 하지 말아야 할 일
- small asteroid를 다시 건드리지 말 것
- large를 다시 랜덤으로 돌리지 말 것
- large / medium에 `progressOffset` 같은 타이밍 꼼수를 다시 넣지 말 것
- opacity fade-in으로 등장 타이밍을 만들지 말 것
- translate 외의 scale 트릭으로 large/medium 존재감을 만들려 하지 말 것
- Scene3 종료 문제를 `행성 앞에서 멈춰 서는` 느낌으로 해결하지 말 것

### 한 줄 요약
현재 Scene3는 **small은 고정, large는 거의 안정화, medium은 문법 정리 중, 남은 큰 과제는 반응형** 상태다. large/medium은 랜덤이 아니라 상수 프리셋으로 다루고, Scene3는 우주선이 멈추는 것이 아니라 **계속 비행하는 가운데 관통 직전에 장면이 끝나는 쪽**이 맞다.
